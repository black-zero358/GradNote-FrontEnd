    ...
    (set, get) => ({
      ...
      addSubmission: (imageFile) => {
        const newSubmission = {
          ...
          reviewStatus: null, // 审核状态：null, PENDING_REVIEW, REVIEWING, REVIEWED
          steps: {
      ...
      // 更新审核状态
      updateReviewStatus: (submissionId, status) => {
        set(state => {
          const submissions = [...state.submissions];
          const index = submissions.findIndex(s => s.id === submissionId);
          ...
          const submission = { ...submissions[index], reviewStatus: status };
          submissions[index] = submission;
          return { submissions };
        });
      },
